...
export default function HtmxChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = htmxChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = htmxChapters[chapterIndex];
  const category = htmxCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = htmxChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? htmxChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < htmxChapters.length - 1
      ? htmxChapters[chapterIndex + 1]
      : null;
