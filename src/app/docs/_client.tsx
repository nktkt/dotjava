...
import { FileCode2, ArrowRight, BookOpen } from "lucide-react";
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      >
        <div className="inline-flex items-center gap-2 mb-4">
          <FileCode2 className="h-8 w-8 text-[var(--color-dads-navy)]" />
          <h1 className="text-3xl md:text-4xl font-bold">
            Java <span className="text-[var(--color-dads-navy)]">API</span> リファレンス
          </h1>
        </div>
        <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
          Java 標準ライブラリの主要クラス・インターフェースを
          コード例付きで体系的に解説
        </p>
        <div className="mt-3 text-sm text-muted-foreground">
          全 {docsChapters.length} チャプター /{" "}
          {docsChapters.reduce((sum, c) => sum + c.sections.length, 0)} セクション
        </div>
      </motion.div>

      {/* Category Sections */}
      {docsCategories.map((category, catIndex) => {
        const chapters = docsChapters.filter(
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {chapters.map((chapter) => (
                <Link key={chapter.id} href={`/docs/${chapter.id}`}>
