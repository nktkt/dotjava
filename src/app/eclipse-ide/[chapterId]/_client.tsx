...
export default function EclipseChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = eclipseChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = eclipseChapters[chapterIndex];
  const category = eclipseCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = eclipseChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? eclipseChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < eclipseChapters.length - 1
      ? eclipseChapters[chapterIndex + 1]
      : null;
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            {prevChapter ? (
              <Link href={`/eclipse-ide/${prevChapter.id}`}>
            ...
            {nextChapter ? (
              <Link href={`/eclipse-ide/${nextChapter.id}`}>
                <Button variant="outline" className="gap-2">
                  <span className="hidden sm:inline">
                    {nextChapter.title}
                  </span>
