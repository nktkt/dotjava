...
export default function ClientPage({
  ...
  const chapterIndex = cicdChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = cicdChapters[chapterIndex];
  const category = cicdCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = cicdChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? cicdChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < cicdChapters.length - 1
      ? cicdChapters[chapterIndex + 1]
      : null;
