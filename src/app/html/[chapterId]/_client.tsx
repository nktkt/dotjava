...
export default function HtmlChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = htmlChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = htmlChapters[chapterIndex];
  const category = htmlCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = htmlChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? htmlChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < htmlChapters.length - 1
      ? htmlChapters[chapterIndex + 1]
      : null;
