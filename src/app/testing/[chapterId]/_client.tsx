...
import { testingChapters, testingCategories } from "@/data/testing";
...
export default function TestingChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = testingChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = testingChapters[chapterIndex];
  const category = testingCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = testingChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? testingChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < testingChapters.length - 1
      ? testingChapters[chapterIndex + 1]
      : null;
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            {prevChapter ? (
              <Link href={`/testing/${prevChapter.id}`}>
            ...
            {nextChapter ? (
              <Link href={`/testing/${nextChapter.id}`}>
                <Button variant="outline" className="gap-2">
                  <span className="hidden sm:inline">
                    {nextChapter.title}
                  </span>
