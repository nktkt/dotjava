      ...
      >
        <div className="inline-flex items-center gap-2 mb-4">
          ...
          <h1 className="text-3xl md:text-4xl font-bold">
            Java <span className="text-[#2563EB]">テスト</span>完全ガイド
          </h1>
        </div>
        <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
          テスト基礎から JUnit 5、Mockito、Spring Boot テスト、
          テスト観点チェックリスト、TDD/BDD まで体系的に学習
        </p>
        <div className="mt-3 text-sm text-muted-foreground">
          全 {testingChapters.length} チャプター /{" "}
          {testingChapters.reduce((sum, c) => sum + c.sections.length, 0)} セクション
        </div>
      </motion.div>

      {/* Category Sections */}
      {testingCategories.map((category, catIndex) => {
        const chapters = testingChapters.filter(
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {chapters.map((chapter) => (
                <Link key={chapter.id} href={`/testing/${chapter.id}`}>
        ...
        <Card className="bg-muted">
          <CardContent className="pt-6">
            ...
            <div className="flex flex-wrap justify-center items-center gap-2">
              {testingCategories.map((cat, index) => (
                <div key={cat.id} className="flex items-center gap-2">
                  ...
                  {index < testingCategories.length - 1 && (
