    ...
    <div className="container mx-auto px-4 py-8">
      ...
      >
        <h1 className="text-3xl md:text-4xl font-bold mb-4">
          OWASPセ<span className="text-[#2563EB]">キュリティ実践</span>
        </h1>
        <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
          OWASP Top 10をJava/Spring Bootで対策する実践ガイド
        </p>
        <div className="mt-3 text-sm text-muted-foreground">
          全 {owaspChapters.length} チャプター /{" "}
          {owaspChapters.reduce((sum, c) => sum + c.sections.length, 0)} セクション
        </div>
      </motion.div>

      {owaspCategories.map((category, catIndex) => {
        const chapters = owaspChapters.filter((c) => c.category === category.id);
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {chapters.map((chapter) => (
                <Link key={chapter.id} href={"/owasp/" + chapter.id}>
