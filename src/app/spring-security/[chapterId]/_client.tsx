...
export default function ClientPage({
  ...
  const chapterIndex = springSecurityChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = springSecurityChapters[chapterIndex];
  const category = springSecurityCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = springSecurityChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? springSecurityChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < springSecurityChapters.length - 1
      ? springSecurityChapters[chapterIndex + 1]
      : null;
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            {prevChapter ? (
              <Link href={`/spring-security/${prevChapter.id}`}>
            ...
            {nextChapter ? (
              <Link href={`/spring-security/${nextChapter.id}`}>
                <Button variant="outline" className="gap-2">
                  <span className="hidden sm:inline">
                    {nextChapter.title}
                  </span>
