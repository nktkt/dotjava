...
import { webChapters, webCategories } from "@/data/java-web";
...
export default function WebChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = webChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = webChapters[chapterIndex];
  const category = webCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = webChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? webChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < webChapters.length - 1
      ? webChapters[chapterIndex + 1]
      : null;
