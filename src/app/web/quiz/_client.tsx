...
import { webQuizQuestions } from "@/data/web-quiz";
import type { WebQuizQuestion, WebLevel } from "@/data/web-quiz";
...
interface QuizProgress {
  ...
  totalXp: Record<WebLevel, number>;
}

const STORAGE_KEY = "web-quiz-progress";

const LEVEL_COLORS: Record<WebLevel, string> = {
  foundation: "var(--color-dads-blue)",
  servlet: "var(--color-dads-success)",
  spring: "var(--color-dads-success)",
  data: "var(--color-dads-warning)",
  security: "var(--color-dads-error)",
  api: "var(--color-dads-purple)",
  testing: "var(--color-dads-cyan)",
  deploy: "var(--color-dads-gray)",
  tomcat: "var(--color-dads-navy)",
};

const LEVEL_LABELS: Record<WebLevel, string> = {
  foundation: "Web基盤",
  servlet: "Servlet/JSP",
  spring: "Spring Boot",
  data: "データアクセス",
...
const LEVEL_EMOJI: Record<WebLevel, string> = {
  foundation: "\uD83C\uDF10",
  servlet: "\u2615",
  spring: "\uD83C\uDF31",
  data: "\uD83D\uDDC4\uFE0F",
...
export default function WebQuizClientPage() {
  ...
  const chaptersForLevel = useMemo(
    () => webChapters.filter((ch) => ch.category === selectedLevel),
    [selectedLevel]
  );

  const questionsForChapter = useCallback(
    (chapterId: string) =>
      webQuizQuestions.filter((q) => q.chapter === chapterId),
    []
  );

  const completedCountForLevel = useCallback(
    (level: WebLevel) => {
      const chapters = webChapters.filter(
  ...
  const startQuiz = useCallback((chapterId: string) => {
    const chapterQuestions = webQuizQuestions.filter(
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        >
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">
            Servlet・Spring Boot・REST API・セキュリティなど、Java Web開発の知識をクイズでテスト。カテゴリを選んで挑戦しましょう。
          </p>
          <div className="mt-2 text-sm text-muted-foreground">
            全 {webQuizQuestions.length} 問収録
          </div>
        </motion.div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {levels.map((level, index) => {
            ...
            // Skip levels with no quiz questions
            const hasQuestions = chapters.some(
              (ch) => webQuizQuestions.filter((q) => q.chapter === ch.id).length > 0
            );
  ...
  const chapterTitle =
    webChapters.find((ch) => ch.id === selectedChapterId)?.title ?? "";
