...
import { certQuizQuestions } from "@/data/java-cert-quiz";
import type { CertQuizQuestion, CertLevel } from "@/data/java-cert-quiz";
...
interface QuizProgress {
  ...
  totalXp: Record<CertLevel, number>;
}

const STORAGE_KEY = "java-cert-quiz-progress";
...
const LEVEL_EMOJI: Record<CertLevel, string> = {
  bronze: "\uD83E\uDD49",
  silver: "\uD83E\uDD48",
  gold: "\uD83E\uDD47",
};
...
function loadProgress(): QuizProgress {
  if (typeof window === "undefined") {
    return { lessons: {}, totalXp: { bronze: 0, silver: 0, gold: 0 } };
  }
  try {
...
export default function JavaCertQuizClientPage() {
  const [phase, setPhase] = useState<Phase>("levelSelect");
  const [selectedLevel, setSelectedLevel] = useState<CertLevel>("bronze");
  const [selectedChapterId, setSelectedChapterId] = useState<string>("");
  const [progress, setProgress] = useState<QuizProgress>(loadProgress);

  // Quiz state
  const [questions, setQuestions] = useState<CertQuizQuestion[]>([]);
  ...
  const chaptersForLevel = useMemo(
    () => javaCertChapters.filter((ch) => ch.category === selectedLevel),
    [selectedLevel]
  );

  const questionsForChapter = useCallback(
    (chapterId: string) =>
      certQuizQuestions.filter((q) => q.chapter === chapterId),
    []
  );

  const completedCountForLevel = useCallback(
    (level: CertLevel) => {
      const chapters = javaCertChapters.filter((ch) => ch.category === level);
  ...
  const startQuiz = useCallback(
    (chapterId: string) => {
      const chapterQuestions = certQuizQuestions.filter(
  ...
  if (phase === "levelSelect") {
    const levels: CertLevel[] = ["bronze", "silver", "gold"];
      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        >
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">
            ...
            の資格試験対策クイズ。レベルを選んで挑戦しましょう。
          </p>
          <div className="mt-2 text-sm text-muted-foreground">
            全 {certQuizQuestions.length} 問収録
          </div>
        </motion.div>
            ...
            >
              ...
              <div className="space-y-3">
                {currentQuestion.choices.map((choice) => {
                  ...
                  let hoverClass = "hover:bg-muted hover:border-muted-foreground/30";
  ...
  const chapterTitle =
    javaCertChapters.find((ch) => ch.id === selectedChapterId)?.title ?? "";
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-lg">
      ...
      <motion.div
        ...
        className="space-y-3"
      >
        <Button onClick={retryQuiz} variant="outline" className="w-full" size="lg">
          <RotateCcw className="h-4 w-4 mr-2" />
          もう一度
        </Button>
        {hasNextChapter && (
