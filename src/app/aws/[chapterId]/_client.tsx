...
export default function AwsChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = awsChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = awsChapters[chapterIndex];
  const category = awsCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = awsChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? awsChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < awsChapters.length - 1
      ? awsChapters[chapterIndex + 1]
      : null;
                ...
                <div className="space-y-1">
                  {sameCategoryChapters.map((c) => (
                    <Link
                      key={c.id}
                      href={`/aws/${c.id}`}
                      className="block px-3 py-2 text-sm rounded-lg hover:bg-[var(--color-dads-warning-light)] transition-colors"
                    >
                      {c.title}
                    </Link>
                  ))}
