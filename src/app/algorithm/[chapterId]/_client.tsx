...
export default function AlgorithmChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = algorithmChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = algorithmChapters[chapterIndex];
  const category = algorithmCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = algorithmChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? algorithmChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < algorithmChapters.length - 1
      ? algorithmChapters[chapterIndex + 1]
      : null;
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            {prevChapter ? (
              <Link href={`/algorithm/${prevChapter.id}`}>
            ...
            {nextChapter ? (
              <Link href={`/algorithm/${nextChapter.id}`}>
                <Button variant="outline" className="gap-2">
                  <span className="hidden sm:inline">
                    {nextChapter.title}
                  </span>
