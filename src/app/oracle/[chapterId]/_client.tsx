...
export default function OracleChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = oracleChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = oracleChapters[chapterIndex];
  const category = oracleCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = oracleChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? oracleChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < oracleChapters.length - 1
      ? oracleChapters[chapterIndex + 1]
      : null;
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            {prevChapter ? (
              <Link href={`/oracle/${prevChapter.id}`}>
            ...
            {nextChapter ? (
              <Link href={`/oracle/${nextChapter.id}`}>
                <Button variant="outline" className="gap-2">
                  <span className="hidden sm:inline">
                    {nextChapter.title}
                  </span>
                ...
                <div className="space-y-1">
                  {sameCategoryChapters.map((c) => (
                    <Link
                      key={c.id}
                      href={`/oracle/${c.id}`}
                      className="block px-3 py-2 text-sm rounded-lg hover:bg-[var(--color-dads-error-light)] transition-colors"
                    >
                      {c.title}
                    </Link>
                  ))}
