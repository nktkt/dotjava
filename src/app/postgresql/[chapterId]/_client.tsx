...
export default function PostgresqlChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = postgresqlChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = postgresqlChapters[chapterIndex];
  const category = postgresqlCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = postgresqlChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? postgresqlChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < postgresqlChapters.length - 1
      ? postgresqlChapters[chapterIndex + 1]
      : null;
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            {prevChapter ? (
              <Link href={`/postgresql/${prevChapter.id}`}>
            ...
            {nextChapter ? (
              <Link href={`/postgresql/${nextChapter.id}`}>
                <Button variant="outline" className="gap-2">
                  <span className="hidden sm:inline">
                    {nextChapter.title}
                  </span>
