...
export default function ClientPage({
  ...
  const chapterIndex = sqlBasicsChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = sqlBasicsChapters[chapterIndex];
  const category = sqlBasicsCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = sqlBasicsChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? sqlBasicsChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < sqlBasicsChapters.length - 1
      ? sqlBasicsChapters[chapterIndex + 1]
      : null;
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            {prevChapter ? (
              <Link href={`/sql-basics/${prevChapter.id}`}>
            ...
            {nextChapter ? (
              <Link href={`/sql-basics/${nextChapter.id}`}>
                <Button variant="outline" className="gap-2">
                  <span className="hidden sm:inline">
                    {nextChapter.title}
                  </span>
