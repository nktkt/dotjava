...
export default function CppLangChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = cppLangChapters.findIndex(
    (c) => c.id === chapterId
  );
  ...
  const chapter = cppLangChapters[chapterIndex];
  const category = cppLangCategories.find(
    (c) => c.id === chapter.category
  );

  const sameCategoryChapters = cppLangChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? cppLangChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < cppLangChapters.length - 1
      ? cppLangChapters[chapterIndex + 1]
      : null;
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            {prevChapter ? (
              <Link href={`/cpp-lang/${prevChapter.id}`}>
            ...
            {nextChapter ? (
              <Link href={`/cpp-lang/${nextChapter.id}`}>
                <Button variant="outline" className="gap-2">
                  <span className="hidden sm:inline">
                    {nextChapter.title}
                  </span>
