...
import { cppLangQuizQuestions } from "@/data/cpp-lang-quiz";
import type {
  CppLangQuizQuestion,
  CppLangLevel,
} from "@/data/cpp-lang-quiz";
...
interface QuizProgress {
  ...
  totalXp: Record<CppLangLevel, number>;
}

const STORAGE_KEY = "cpp-lang-quiz-progress";

const LEVEL_COLORS: Record<CppLangLevel, string> = {
...
const LEVEL_LABELS: Record<CppLangLevel, string> = {
  basics: "基礎文法",
  intermediate: "中級",
  advanced: "上級",
  expert: "実践・応用",
...
const LEVEL_EMOJI: Record<CppLangLevel, string> = {
  basics: "\uD83D\uDCDD",
  intermediate: "\uD83D\uDD27",
  advanced: "\u26A1",
  expert: "\uD83C\uDFC6",
...
export default function CppLangQuizClientPage() {
  const [phase, setPhase] = useState<Phase>("levelSelect");
  const [selectedLevel, setSelectedLevel] =
    useState<CppLangLevel>("basics");
  const [selectedChapterId, setSelectedChapterId] = useState<string>("");
  const [progress, setProgress] = useState<QuizProgress>(loadProgress);

  // Quiz state
  const [questions, setQuestions] = useState<CppLangQuizQuestion[]>([]);
  ...
  const chaptersForLevel = useMemo(
    () => cppLangChapters.filter((ch) => ch.category === selectedLevel),
    [selectedLevel]
  );

  const questionsForChapter = useCallback(
    (chapterId: string) =>
      cppLangQuizQuestions.filter((q) => q.chapter === chapterId),
    []
  );
  ...
  const startQuiz = useCallback((chapterId: string) => {
    const chapterQuestions = cppLangQuizQuestions.filter(
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        >
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">
            C++の基礎文法・中級・上級・実践の知識をクイズでテスト。レベルを選んで挑戦しましょう。
          </p>
          <div className="mt-2 text-sm text-muted-foreground">
            全 {cppLangQuizQuestions.length} 問収録
          </div>
        </motion.div>
  ...
  const chapterTitle =
    cppLangChapters.find((ch) => ch.id === selectedChapterId)?.title ?? "";
