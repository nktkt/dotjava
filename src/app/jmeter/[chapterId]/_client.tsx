...
export default function ClientPage({
  ...
  const chapterIndex = jmeterChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = jmeterChapters[chapterIndex];
  const category = jmeterCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = jmeterChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? jmeterChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < jmeterChapters.length - 1
      ? jmeterChapters[chapterIndex + 1]
      : null;
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            {prevChapter ? (
              <Link href={`/jmeter/${prevChapter.id}`}>
            ...
            {nextChapter ? (
              <Link href={`/jmeter/${nextChapter.id}`}>
                <Button variant="outline" className="gap-2">
                  <span className="hidden sm:inline">
                    {nextChapter.title}
                  </span>
