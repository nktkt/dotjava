...
export default function SecurityChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = securityChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = securityChapters[chapterIndex];
  const category = securityCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = securityChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? securityChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < securityChapters.length - 1
      ? securityChapters[chapterIndex + 1]
      : null;
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            {prevChapter ? (
              <Link href={`/security/${prevChapter.id}`}>
            ...
            {nextChapter ? (
              <Link href={`/security/${nextChapter.id}`}>
                <Button variant="outline" className="gap-2">
                  <span className="hidden sm:inline">
                    {nextChapter.title}
                  </span>
