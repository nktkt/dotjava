...
import { securityQuizQuestions } from "@/data/security-quiz";
import type { SecurityQuizQuestion, SecurityLevel } from "@/data/security-quiz";
...
interface QuizProgress {
  ...
  totalXp: Record<SecurityLevel, number>;
}

const STORAGE_KEY = "security-quiz-progress";

const LEVEL_COLORS: Record<SecurityLevel, string> = {
  basics: "var(--color-dads-navy)",
  auth: "var(--color-dads-blue)",
  crypto: "var(--color-dads-purple)",
  web: "var(--color-dads-error)",
  coding: "var(--color-dads-success)",
  ops: "var(--color-dads-warning)",
};
...
const LEVEL_EMOJI: Record<SecurityLevel, string> = {
  basics: "\uD83D\uDEE1\uFE0F",
  auth: "\uD83D\uDD11",
  crypto: "\uD83D\uDD10",
  web: "\uD83C\uDF10",
...
export default function SecurityQuizClientPage() {
  const [phase, setPhase] = useState<Phase>("levelSelect");
  const [selectedLevel, setSelectedLevel] =
    useState<SecurityLevel>("basics");
  const [selectedChapterId, setSelectedChapterId] = useState<string>("");
  const [progress, setProgress] = useState<QuizProgress>(loadProgress);

  // Quiz state
  const [questions, setQuestions] = useState<SecurityQuizQuestion[]>([]);
  ...
  const chaptersForLevel = useMemo(
    () => securityChapters.filter((ch) => ch.category === selectedLevel),
    [selectedLevel]
  );

  const questionsForChapter = useCallback(
    (chapterId: string) =>
      securityQuizQuestions.filter((q) => q.chapter === chapterId),
    []
  );
  ...
  const startQuiz = useCallback((chapterId: string) => {
    const chapterQuestions = securityQuizQuestions.filter(
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        >
          <div className="inline-flex items-center gap-2 mb-4">
            <Trophy className="h-8 w-8 text-[var(--color-dads-navy)]" />
            <h1 className="text-3xl md:text-4xl font-bold">
              Javaセキュリティクイズ
            </h1>
          </div>
          <p className="text-muted-foreground max-w-xl mx-auto">
            暗号化・認証認可・XSS・CSRF・Spring Securityなど、Javaセキュリティの知識をクイズでテスト。カテゴリを選んで挑戦しましょう。
          </p>
          <div className="mt-2 text-sm text-muted-foreground">
            全 {securityQuizQuestions.length} 問収録
          </div>
        </motion.div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {levels.map((level, index) => {
            ...
            const hasQuestions = chapters.some(
              (ch) => securityQuizQuestions.filter((q) => q.chapter === ch.id).length > 0
            );
  ...
  const chapterTitle =
    securityChapters.find((ch) => ch.id === selectedChapterId)?.title ?? "";
