    ...
    <div className="container mx-auto px-4 py-8">
      ...
      >
        <h1 className="text-3xl md:text-4xl font-bold mb-4">
          データベー<span className="text-[#2563EB]">ス設計ガイド</span>
        </h1>
        <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
          正規化、ER図、インデックス設計、クエリチューニング、トランザクション管理まで体系的に学習
        </p>
        <div className="mt-3 text-sm text-muted-foreground">
          全 {dbDesignChapters.length} チャプター /{" "}
          {dbDesignChapters.reduce((sum, c) => sum + c.sections.length, 0)} セクション
        </div>
      </motion.div>

      {dbDesignCategories.map((category, catIndex) => {
        const chapters = dbDesignChapters.filter((c) => c.category === category.id);
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {chapters.map((chapter) => (
                <Link key={chapter.id} href={"/db-design/" + chapter.id}>
