...
export default function CssChapterClientPage({
  chapterId,
}: {
  chapterId: string;
}) {
  const chapterIndex = cssChapters.findIndex((c) => c.id === chapterId);

  if (chapterIndex === -1) {
    notFound();
  }

  const chapter = cssChapters[chapterIndex];
  const category = cssCategories.find((c) => c.id === chapter.category);

  const sameCategoryChapters = cssChapters.filter(
    (c) => c.category === chapter.category && c.id !== chapter.id
  );

  const prevChapter =
    chapterIndex > 0 ? cssChapters[chapterIndex - 1] : null;
  const nextChapter =
    chapterIndex < cssChapters.length - 1
      ? cssChapters[chapterIndex + 1]
      : null;
