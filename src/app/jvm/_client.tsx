    ...
    <div className="container mx-auto px-4 py-8">
      ...
      >
        <h1 className="text-3xl md:text-4xl font-bold mb-4">
          JVMメモ<span className="text-[#2563EB]">リ管理ガイド</span>
        </h1>
        <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
          JVMアーキテクチャ、GCアルゴリズム、メモリリーク調査、プロファイリングまで体系的に学習
        </p>
        <div className="mt-3 text-sm text-muted-foreground">
          全 {jvmChapters.length} チャプター /{" "}
          {jvmChapters.reduce((sum, c) => sum + c.sections.length, 0)} セクション
        </div>
      </motion.div>

      {jvmCategories.map((category, catIndex) => {
        const chapters = jvmChapters.filter((c) => c.category === category.id);
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {chapters.map((chapter) => (
                <Link key={chapter.id} href={"/jvm/" + chapter.id}>
