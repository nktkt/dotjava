    ...
    <div className="container mx-auto px-4 py-8">
      ...
      >
        <h1 className="text-3xl md:text-4xl font-bold mb-4">
          Spring<span className="text-[#2563EB]">Bootテスト完全ガイド</span>
        </h1>
        <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
          JUnit 5、Mockito、MockMvc、Testcontainersを使ったSpring Bootテストを体系的に学習
        </p>
        <div className="mt-3 text-sm text-muted-foreground">
          全 {springBootTestingChapters.length} チャプター /{" "}
          {springBootTestingChapters.reduce((sum, c) => sum + c.sections.length, 0)} セクション
        </div>
      </motion.div>

      {springBootTestingCategories.map((category, catIndex) => {
        const chapters = springBootTestingChapters.filter((c) => c.category === category.id);
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {chapters.map((chapter) => (
                <Link key={chapter.id} href={"/spring-boot-testing/" + chapter.id}>
