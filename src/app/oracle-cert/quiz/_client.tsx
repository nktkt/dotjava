...
import { oracleCertQuizQuestions } from "@/data/oracle-cert-quiz";
import type {
  OracleCertQuizQuestion,
  OracleCertLevel,
} from "@/data/oracle-cert-quiz";
...
interface QuizProgress {
  ...
  totalXp: Record<OracleCertLevel, number>;
}

const STORAGE_KEY = "oracle-cert-quiz-progress";

const LEVEL_COLORS: Record<OracleCertLevel, string> = {
  "silver-sql": "#2196F3",
  "bronze-dba": "#CD7F32",
  "silver-dba": "#9E9E9E",
  "gold-dba": "#FFC107",
...
const LEVEL_LABELS: Record<OracleCertLevel, string> = {
  "silver-sql": "Silver SQL",
  "bronze-dba": "Bronze DBA",
  "silver-dba": "Silver DBA",
  "gold-dba": "Gold DBA",
};

const LEVEL_EMOJI: Record<OracleCertLevel, string> = {
  "silver-sql": "\uD83D\uDCDD",
  "bronze-dba": "\uD83E\uDD49",
  "silver-dba": "\uD83E\uDD48",
  "gold-dba": "\uD83E\uDD47",
...
export default function OracleCertQuizClientPage() {
  const [phase, setPhase] = useState<Phase>("levelSelect");
  const [selectedLevel, setSelectedLevel] =
    useState<OracleCertLevel>("silver-sql");
  const [selectedChapterId, setSelectedChapterId] = useState<string>("");
  const [progress, setProgress] = useState<QuizProgress>(loadProgress);

  // Quiz state
  const [questions, setQuestions] = useState<OracleCertQuizQuestion[]>([]);
  ...
  const chaptersForLevel = useMemo(
    () => oracleCertChapters.filter((ch) => ch.category === selectedLevel),
    [selectedLevel]
  );

  const questionsForChapter = useCallback(
    (chapterId: string) =>
      oracleCertQuizQuestions.filter((q) => q.chapter === chapterId),
    []
  );

  const completedCountForLevel = useCallback(
    (level: OracleCertLevel) => {
      const chapters = oracleCertChapters.filter(
  ...
  const startQuiz = useCallback((chapterId: string) => {
    const chapterQuestions = oracleCertQuizQuestions.filter(
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        >
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">
            ORACLE MASTER Silver SQL・Bronze DBA・Silver DBA・Gold
            DBAの資格試験対策クイズ。レベルを選んで挑戦しましょう。
          </p>
          <div className="mt-2 text-sm text-muted-foreground">
            全 {oracleCertQuizQuestions.length} 問収録
          </div>
        </motion.div>
  ...
  const chapterTitle =
    oracleCertChapters.find((ch) => ch.id === selectedChapterId)?.title ?? "";
