...
export default function OracleCertClientPage() {
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      >
        ...
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          ...
          DBAの資格試験対策。出題範囲を体系的にカバーし、頻出パターンと模擬問題で合格力を養成します。
        </p>
      </motion.div>

      {/* Categories */}
      <div className="space-y-12 mb-16">
        {oracleCertCategories.map((category, catIndex) => {
          ...
          <div className="inline-flex flex-col items-center gap-4 p-8 rounded-2xl border-2 border-dashed border-[var(--color-dads-blue)] bg-[var(--color-dads-blue-light)] hover:bg-[var(--color-dads-blue)]/10 transition-colors cursor-pointer">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Duolingo風クイズでSilver SQL・Bronze DBA・Silver DBA・Gold
                DBAの知識をテスト
              </p>
            </div>
          </div>
      ...
      >
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto mb-6">
          まずSilver
          SQLでSQL基礎を固め、Bronze・Silver・Gold
          DBAとデータベース管理のスキルを段階的に習得しましょう。各レベルの出題傾向を把握し、実機での演習を重ねることが合格への近道です。
        </p>
        <div className="flex flex-wrap justify-center gap-2">
          {oracleCertCategories.map((cat) => (
            <Badge
              key={cat.id}
              variant="outline"
